  ...
  return (
    <div className="comanda">
      <h4 className="comanda__mesa">
        {mesa_global ? `Mesa ${mesa_global.mesa_nro}` : "Sin mesa seleccionada"}
      </h4>
      ...
      {!mesa_global ? (
        ...
        <>
          <ul className="comanda__lista">
            {pedidoActual.platos.map((objPlato) => {
              return (
                <PosComandaItem objPlato={objPlato} key={objPlato.plato_id} />
              );
            })}
          </ul>
          <button
        ...
        <div>No hay pedidos en la mesa {mesa_global.mesa_nro} 😥</div>
      )}
