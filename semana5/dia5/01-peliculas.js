...
/**
 * Funcion que agrega el atributo generos que es un arreglo7
 * el cual contien los generos de las peliculas.
 */
...
const dibujarTarjetas = () => {
  peliculas.forEach((peli) => {
    ...
    peli.generos.forEach((gen) => {
      botonesGeneros = botonesGeneros + `<p>${gen}</p>`;
    });

    let card = document.createElement("div");
    card.classList.add("main__card");
    card.innerHTML = `<figure class="card__figure">
              <img
                class="main__img"
                src="http://placehold.it/200x200/444444/bbbbbb?text=logo1 -
            300x300"
                alt=""
              />
            </figure>
            <div class="card__info">
              <h3 class="card__titulo"> ${peli.title}</h3>
              <p class="card__fecha">${peli.release_date}</p>
...
const dibujarCategorias = () => {
  generos.forEach((gen) => {
    let catNombre = document.createElement("p");
    catNombre.classList.add("aside__link");
    catNombre.innerText = gen.name.toUpperCase();
    aside.appendChild(catNombre);
  });
};
